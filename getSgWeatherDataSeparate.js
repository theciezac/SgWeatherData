...
function logToDataFile(content) {
    fs.appendFile("newdata.log", content + "\r\n", function (err) {
        if (err) throw err;
    });
}
...
fs.readFile("time.txt", function (err, data) {
    ...
    if (now.getTime() >= startTime.getTime()) {
        console.log("Database is not updated. Please run getOldSgWeatherDataSeparate.js first.");
        process.exit();
    }

    console.log("Database is updated. Starting process...");
